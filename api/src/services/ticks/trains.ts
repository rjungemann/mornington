import db, { Hop, Station, Train } from '../../models';
import { Model } from 'sequelize';
import { logger } from '../../logging'
import { ClockContext } from '../../types';
import { gameSeededRandom } from '.././ticks/shared';

async function tickTravelingTrainEndOfHop(train: Model<Train>, hop: Model<Hop>, context: ClockContext) {
...
async function tickTravelingTrain(train: Model<Train>, context: ClockContext) {
  ...
  if (train.dataValues.distance >= hop.dataValues.length) {
    await tickTravelingTrainEndOfHop(train, hop, context)
    return
  }

  // Check for overtaken trains
  //
  // Filter out current train
  // Only consider trains of the same line
...
async function tickStationedTrainDeparting(train: Model<Train>, station: Model<Station>, context: ClockContext) {
  const { game, lines, trains, hops, stations, agents, hazards, items } = context
  const { id: gameId, name: gameName, turnNumber, currentTime } = game.dataValues
...
async function tickStationedTrain(train: Model<Train>, context: ClockContext) {
  ...
  if (station.dataValues.virtual) {
    await tickStationedTrainDepartVirtualStation(train, station, context)
    return
  }

  if (train.dataValues.currentWaitTime >= train.dataValues.maxWaitTime) {
    await tickStationedTrainDeparting(train, station, context)
    return
  }
